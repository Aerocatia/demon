...
pub unsafe fn play_looping_sound(tag_id: TagID, object_id: ID<0x626F>, gain: f32) {
    const SCRIPT_LOOPING_SOUND_START: PointerProvider<unsafe extern "C" fn(TagID, ID<0x626F>, f32)> = PointerProvider {
...
pub unsafe fn stop_looping_sound(tag_id: TagID) {
    const SCRIPT_LOOPING_SOUND_STOP: PointerProvider<unsafe extern "C" fn(TagID)> = PointerProvider {
