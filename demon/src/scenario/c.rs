...
pub unsafe extern "C" fn ai_index_from_string(scenario: &mut Scenario, ai_string: CStrPtr, ai_index_reference: &mut u32) -> bool {
    let ai_string = ai_string.expect_str();
    *ai_index_reference = 0xFFFFFFFF;
    ...
    let Some((encounter, subencounter)) = ai_string.split_once('/') else {
        let Some(encounter_index) = get_encounter_by_name(ai_string) else { return false };
        *ai_index_reference = encounter_index;
        return true
    };

    let Some(encounter_index) = get_encounter_by_name(encounter) else { return false };
    ...
    *ai_index_reference = (upper as u32) | (encounter_index);
    true
}
