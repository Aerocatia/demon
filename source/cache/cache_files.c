...
int32_t scenario_tags_load(const char *name) {
    ...
    auto scenario = scenario_get(cache_file_globals.tags_header->scenario_tag_index);
    ...
    while((tag_index = tag_iterator_next(&iterator)) != NONE) {
        auto tag_instance = cache_file_tag_instance_get(tag_index);
        ...
        switch(tag_instance->group_tag) {
            ...
            case SOUND_DEFINITION_TAG:
                auto sound_index = data_file_find_item(_data_file_type_sound, tag_get_name(tag_index));
                size = data_file_load_tag(_data_file_type_sound, sound_index, tag_data_cursor);
                assert(size);

                auto sound = sound_definition_get(tag_index);
                auto loaded_sound = (struct sound_definition *)tag_data_cursor;
...
int32_t tag_iterator_next(struct tag_iterator *iterator) {
    while(iterator->iterator.absolute_index < cache_file_globals.tags_header->tag_count) {
        auto tag_instance = &global_tag_instances[iterator->iterator.absolute_index++];
        if(tag_instance) {
            if(
